...
$query = "SELECT id, type, customer,cel_phone, cnee,cnee_cel_phone,address, nro, location, referencia, inscription, product, add1, add2, add3, add4, add5, add6, schedule_available, status FROM `general` WHERE `delivery_date` <= (DATE_ADD(CURDATE(), INTERVAL 6 DAY)) AND delivery_date >= CURDATE() AND WEEKDAY(`delivery_date`) = 5";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "PicadasSabado_$date.xls";
